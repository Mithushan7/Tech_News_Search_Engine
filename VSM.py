...
def compute_tfidf(inverted_index, num_documents):
    ...
    for term, postings in inverted_index.items():
        doc_frequency = len(postings)  #num of documents containing the term
        idf[term] = np.log(num_documents / (doc_frequency + 1))  #smoothing to prevent division by zero

    #TF-IDF for each document
    for term, postings in inverted_index.items():
        for doc_id in postings:
            tf = postings[doc_id][0]  
...
def create_document_vectors(tfidf, all_terms):
    ...
    for doc_id, terms in tfidf.items():
        #zero vector of length equal to the number of unique terms
...
def compute_query_vector(query, inverted_index, idf, all_terms):
    ...
    for term in query_terms:
        if term in inverted_index:
            ...
            query_vector[term_index] = 1 * idf[term]  #1 ,nothing change, just for clarity
    return query_vector
...
if __name__ == "__main__":
    ...
    #ranking documents based on the query vector
